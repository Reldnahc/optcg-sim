...
ID: SUP-003A
Epic ID: SUP-003
Title: Optional choose-one trash cost contract authorability
Type: implementation
Area: contracts
...
Authorize optional choose-one cost segments in the shared Effect DSL so engine sequence runtime can represent text shaped like `You may trash A or B:` as a multi-option optional cost decision rather than an exact printed sentence.
...
- add optional choose-one cost authorability for `{ type: "payCost"; cost: OptionalCost }` sequence segments
- consume the existing SUP-002A optional `trashFromHand` authorability as one allowed alternative rather than reopening or duplicating that contract
- allow the scoped optional choose-one cost to contain only the reusable SUP-002A optional `trashFromHand` branch and the new scoped optional self Character-field `trashFromField` branch
...
- legal action generation for optional choose-one costs
- parser, generated-support, runtime capability matrix, support reports, overlays, fixture capture, or card promotion
- optional cost alternatives beyond scoped `trashFromField` and `trashFromHand`
- arbitrary nested choose-one costs, comma-separated printed grammar, discard aliases, opponent-field costs, or broad optional-cost grammar

## Allowed Touch Points

<!-- prettier-ignore -->
- stories/generated/SUP-003A-optional-choose-one-trash-cost-contract-authorability.yaml
- stories/approved/SUP-003A-optional-choose-one-trash-cost-contract-authorability.yaml
- agent-packets/SUP-003A.md
...
- `OptionalCost` in specs, contracts, and package types can express a reusable optional choose-one cost with scoped field-trash and reused SUP-002A hand-trash alternatives
- optional field-trash alternatives require a positive integer count, chooser self, literal optional semantics, and a scoped self Character `typesAny` filter; broader field zones, opponent field costs, non-Character categories, and arbitrary filters remain rejected
- optional choose-one alternatives cannot be empty, cannot include unsupported cost families, and cannot blur optional cost with optional activation
- `05-effect-dsl-reference.s029` distinguishes scoped optional choose-one trash-cost authorability from still-planned broad non-optional `Cost.chooseOne` and `Cost.trashFromField` authorability
