...
<!-- agent-packet:story-sha256 5f05b0895f3d5ea0850aca3bd91dbbe4d5b29d002be3ddafe628e64d976f6bca -->
...
Own only selectTargets decision creation for supported target requests after candidate resolution. Do not validate response payloads or resolve target effects.
...
- expose a legal respondToDecision action only to the decision player while a selectTargets decision is pending
...
- if decision creation requires response validation, resolving target effects, optional activation, replacement effects, or custom handlers, stop and split or record the blocker
...
- run corepack pnpm exec vitest run packages/engine-core/src/target-selection.test.ts packages/engine-core/src/effect-runtime.test.ts packages/engine-core/src/actions.test.ts packages/engine-core/src/filter-state-for-player.test.ts
...
- getLegalActions exposes respondToDecision only to the chooser while the decision is pending
