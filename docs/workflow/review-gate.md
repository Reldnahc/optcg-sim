...
15. run the cleanup metadata handoff preflight against the actual current PR body or selected durable handoff comment, fetched changed files, fetched PR head branch, fetched status checks, and reviewed PR evidence, not a copied example or reconstructed local text; use `node --experimental-strip-types tools/post-merge-cleanup.ts -- --validate-cleanup-handoff-json-file <handoff.json> --require-cleanup-guard-status` when fetched PR metadata and check status are available
...
19. for parent/substory workflows, confirm the Story Approval Review Gate story-set review-status matrix is reconstructed from durable artifacts before PR opening or PR handoff; fail closed when the parent-story review is unknown or pending, or when any child-story review is missing, unknown, or pending; require distinct story-review assignment identity and distinct durable artifact identity per required row; one story-review assignment, one reviewer run, one matrix, or one durable artifact covering multiple stories satisfies at most one required row; batch story-review is supplemental context only and cannot satisfy multiple required rows; also fail closed when any row is not reconstructable from durable evidence
