...
- Before human review, compute the exact cleanup metadata source ref with `corepack pnpm cleanup:validate-dry-run -- --print-source-ref --metadata-source-kind pr-body --metadata-source-id <source-id> --metadata-source-file <file>` for PR-body metadata or `--metadata-source-kind handoff-comment --metadata-source-id <comment-id>` for a durable handoff comment. The source ref must be `pr-body:<source-id>:<sha256>` or `handoff-comment:<comment-id>:<sha256>` and must not be embedded in the source being hashed.
- Human merge-gate approval or the equivalent human-review fallback must reference the exact cleanup metadata source ref. Text that merely repeats the ref is guidance; the PR body or durable handoff comment content is the reviewed cleanup metadata source.
