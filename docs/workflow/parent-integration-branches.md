...
- Parent PR cleanup metadata may live in the PR body or in a durable handoff comment whose content is the reviewed cleanup metadata source. Compute and publish the exact cleanup metadata source ref before review (`pr-body:<source-id>:<sha256>` or `handoff-comment:<comment-id>:<sha256>`); the human merge-gate approval or equivalent fallback must reference that exact ref.
