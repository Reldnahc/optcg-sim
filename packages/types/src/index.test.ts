...
test("TYP-001H ownership manifest keys exactly match canonical export type/interface names", async () => {
  const canonicalPath = new URL("../../../contracts/canonical-types.ts", import.meta.url);
  const canonicalSource = await import("node:fs/promises").then((fs) =>
    fs.readFile(canonicalPath, "utf8"),
  );

  const canonicalNames = [...canonicalSource.matchAll(/export\s+(?:type|interface)\s+([A-Za-z0-9_]+)/g)]
    .map((match) => match[1])
    .sort();
...
test("TYP-001H relative runtime import/object-key smoke proof is not sufficient by itself", async () => {
  ...
  const packageNameModule = await import("@optcg/types");
  expect(Object.keys(relativeModule)).toEqual([]);
  expect(Object.keys(packageNameModule)).toEqual([]);
