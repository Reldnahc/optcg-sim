...
export const validateRestReplacementTarget = (
  state: GameState,
  effectId: string,
  target: CardRef,
):
  ...
  if (
    located.zone !== "leaderArea" &&
    located.zone !== "characterArea" &&
    located.zone !== "stageArea" &&
    located.zone !== "costArea"
  ) {
    return failure(effectId, "stale-target");
  }
  ...
  if (resolved === undefined) return failure(effectId, "missing-card");
  if (
    (located.zone === "leaderArea" && resolved.category !== "leader") ||
    (located.zone === "characterArea" && resolved.category !== "character") ||
    (located.zone === "stageArea" && resolved.category !== "stage") ||
    (located.zone === "costArea" && resolved.category !== "don")
  ) {
    return failure(effectId, "non-character-target");
  }
  return { ok: true, located, ref, resolved };
...
export const validateRestReplacementTargets = (
  state: GameState,
  effectId: string,
  targets: readonly CardRef[],
):
  ...
  const validated: ValidatedReplacementTarget[] = [];
  for (const target of targets) {
    const lookup = validateRestReplacementTarget(state, effectId, target);
    if (!lookup.ok) {
      return lookup;
    }
    validated.push(lookup);
