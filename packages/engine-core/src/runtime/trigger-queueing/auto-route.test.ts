...
import type { EffectDefinition, Trigger } from "@optcg/types";
...
const setupAutoLifeRemovedDefinition = (
  state: ReturnType<typeof createActiveState>,
  trigger: Trigger,
): void => {
  const p1State = must(state.players[p1], "p1");
  const source = withCardInZone({
    state,
    playerId: p1,
    card: must(p1State.hand[0], "auto source"),
    zone: "characterArea",
  });
  p1State.hand = p1State.hand.filter(
    (card) => card.instanceId !== source.instanceId,
  ...
  const supportCard = resolvedCard({
    ...
    support: {
      status: "implemented-dsl",
      effectDefinitionId: "def-auto-life-removed",
      rulesVersion: "auto-life-removed-rules",
      sourceTextHash: "auto-life-removed-source",
    },
  });
  const baseDefinition = reviewedOnPlayDrawDefinition(
    source.cardId,
  ...
  const definition: EffectDefinition = {
    ...
    effects: [
      {
        ...must(baseDefinition.effects[0], "base draw effect"),
        id: toEffectId("auto-life-removed-draw"),
        trigger,
        sourcePresencePolicy: "mustRemainInSameZone",
        effect: {
          type: "sequence",
  ...
  state.cardManifest.effectDefinitionsVersion =
    definition.metadata.effectDefinitionsVersion;
  state.cardManifest.effectDefinitions = {
    "def-auto-life-removed": definition,
  };
  state.cardManifest.cards[source.cardId] = supportCard;
};
...
test("automatic lifeRemoved queueing uses canonical event matching for composed triggers", () => {
  ...
  state.turn.turnPlayerId = p1;
  state.eventJournal = [];
  setupAutoLifeRemovedDefinition(state, {
    type: "anyOf",
    triggers: [
  ...
  const movedLife = must(before.life[0], "life card").card;
  state.eventJournal.push({
    id: toEngineEventId("event:auto-life-removed:public"),
    seq: 1,
    type: "cardMoved",
    payload: {
      from: { zone: "life", playerId: p1, slot: "life", index: 0 },
  ...
  state.eventJournal.push({
    id: toEngineEventId("event:auto-life-removed:private"),
    seq: 2,
    type: "cardMoved",
    payload: {
      instanceId: movedLife.instanceId,
  ...
  assert.equal(
    queued.state.effectQueue[0]?.effectBlockId,
    "auto-life-removed-draw",
  );
});
