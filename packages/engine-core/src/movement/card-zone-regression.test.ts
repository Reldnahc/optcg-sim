...
test("set trash remainder delegates concrete movement", () => {
  const source = readSource(
  ...
  assert.doesNotMatch(
    source,
    /appendEvent\([^)]*"cardTrashed"/s,
    "set trash remainder must not hand-roll cardTrashed",
  );
  assert.doesNotMatch(
    source,
    /zone:\s*\{\s*zone:\s*"trash"/s,
    "set trash remainder must not construct trash-zone cards directly",
  );
});
