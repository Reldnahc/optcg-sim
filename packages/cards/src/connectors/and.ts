...
const andPattern =
  /\s*,?\s+and\s+(?!different card names\b|(?:Leader|Character|Event|Stage)s? effects\b|your opponent's Life cards\b|set (?:it|them) as active\b|rest (?:it|them)\b|\+\d+\s+(?:cost|power)\b|place them at the top(?: or bottom)?\b)/iu;
