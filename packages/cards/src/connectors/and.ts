...
export const parseAndConnector: ConnectorParser = (input) => {
  const segments = input.text
    .split(
      /\s*,?\s+and\s+(?!different card names\b|(?:Leader|Character|Event|Stage)s? effects\b|set it as active\b|rest them\b|\+\d+\s+(?:cost|power)\b|place them at the top(?: or bottom)?\b)/i,
    )
