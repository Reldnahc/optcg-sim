...
describe("card action menu", () => {
  ...
  test("card styling removes the default border while state feedback hugs the tile", async () => {
    ...
    assert.match(
      appShellStyles,
      /\.match-app\s*\{[^}]*--card-corner-radius:\s*calc\(var\(--card-height\) \/ 28\);[^}]*--card-outline-heavy:\s*calc\(var\(--card-height\) \/ 28\);[^}]*--card-outline-medium:\s*calc\(var\(--card-height\) \/ 24\);[^}]*--card-outline-thin:\s*calc\(var\(--card-height\) \/ 42\);/u,
    );
    assert.match(
      styles,
