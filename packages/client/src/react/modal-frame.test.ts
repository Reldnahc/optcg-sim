...
describe("modal frame", () => {
  ...
  test("modal frame defaults to compact content sizing with viewport guardrails", async () => {
    ...
    assert.match(
      styles,
      /\.modal-frame\s*\{[^}]*--modal-viewport-gutter:\s*clamp\(16px,\s*calc\(var\(--card-height\) \/ 4\),\s*48px\);[^}]*--modal-padding:\s*clamp\(12px,\s*calc\(var\(--card-height\) \/ 8\),\s*24px\);[^}]*--modal-gap:\s*clamp\(8px,\s*calc\(var\(--card-height\) \/ 14\),\s*16px\);[^}]*--modal-card-decision-inline-extra:\s*calc\([^}]*\(var\(--card-outline-thin\) \* 2\)/u,
    );
    assert.match(
      styles,
