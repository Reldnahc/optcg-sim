...
const sha256Fallback = (bytes: Uint8Array): ArrayBuffer => {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const ch = (e & f) ^ (~e & g);
      const temp1 = (h + s1 + ch + (k[index] ?? 0) + (words[index] ?? 0)) >>> 0;
...
export const requestHash = async (request: unknown): Promise<string> => {
  ...
  const digest =
    typeof subtleDigest === "function"
      ? await subtleDigest.call(subtle, "SHA-256", encoded)
