...
const lethalCounterDecisionSnapshot = (): DevMatchSnapshot => {
...
describe("chooseDefaultBotDecision", () => {
  test("uses enough counter cards to stop lethal leader attacks", () => {
    const chosen = chooseDefaultBotDecision({
      snapshot: lethalCounterDecisionSnapshot(),
      botPlayerId,
    });

    assert.deepEqual(
      chosen?.response.type === "cards"
        ? chosen.response.cards.map((card) => String(card.instanceId))
        : [],
