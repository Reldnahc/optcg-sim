...
const lifePositionLabel = (
  position: "top" | "bottom" | "topOrBottom" | "anyMatching" | undefined,
): string => {
  switch (position) {
    case "top":
      return "top Life";
...
const paymentOptionLabel = (
  ...
  switch (option.type) {
    ...
    case "restFromField":
      return `Rest ${countLabel(selectedCardsCount || option.count, "card", "cards")} from field`;
    case "trashFromHand":
